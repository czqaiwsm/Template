...
public class UpdateMgr {
	...
	public class onAdviceUpdateEvent implements UpdateEventCallback {
		...
		@Override
		public void onUpdateFailEvent() {
			toasetUtil.showErro(mContext.getString(R.string.update_failed));
		}
...
/*
			*//*******1.start---模仿请求接口*********//*
			...
			updateInfo.setDownloadUrl("http://meishigyy.com/apk/source/xdfood.apk");
...
*/
			...
			postMap.put("version",pInfo.versionCode+"");
			AppLog.Logi("versionCode:" + pInfo.versionCode);
			url.setmBaseUrl(Constant.VERSION);
			param.setmHttpURL(url);
			param.setmPostMap(postMap);
			...
			public void onResponse(Object object) {
				if (object != null) {
					...
					if (jsonParserBase != null && Constant.SUCCESS_CODE.equals(jsonParserBase.getCode())) {
						JsonParserBase<UpdateInfo> base = ParserUtil.fromJsonBase(object.toString(), new TypeToken<JsonParserBase<UpdateInfo>>() {
						}.getType());
						UpdateInfo updateInfo = base.getData();
						if(updateInfo != null && !TextUtils.isEmpty(updateInfo.getDownloadUrl())){
							IS_FORCE_UPDATE = updateInfo.getIsForceDown();

							if(autoUpdate){
								IS_FORCE_UPDATE = false;
								updateInfo.setUpdateMode(UpdateMode.USER_SELECT);
							}
							update(updateInfo, autoUpdate);

						}else {
							if(autoUpdate){
							  toasetUtil.showInfo("您当前已是最高版本!");
							}
						}
					}else {
						if(autoUpdate){
							toasetUtil.showInfo(jsonParserBase.getData()+"");
						}
					}
				}
			}
