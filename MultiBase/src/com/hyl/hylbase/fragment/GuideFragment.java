...
import com.hyl.hylbase.activity.MainActivity;
...
public class GuideFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		return inflater.inflate(R.layout.guide_base, null);
	}
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		((BannerViewAdapter)m_obj_adapter).setAutoPlay(m_obj_viewPager,false);
		m_obj_viewPager.setOnPageChangeListener(new OnPageChangeListener() {

			@Override
			public void onPageSelected(int position) {
				int pos = position % ids.length;
				((BannerViewAdapter) m_obj_adapter).moveCursorTo(pos);// 点的移动
				if (pos == ids.length - 1) {
					((BannerViewAdapter)m_obj_adapter).setAutoPlay(m_obj_viewPager,false);
	...
	private void initView() {
		m_obj_adapter = new BannerViewAdapter(ids, m_obj_Layoutview,
				getActivity());
	}
