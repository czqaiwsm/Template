...
import com.hyl.hylbase.utils.WaitLayer;
import com.toast.ToasetUtil;
import com.volley.req.RequsetListener;
 ...
 * @data 2016/3/10
 */
public abstract class MultiBaseFragment extends BaseFragment{
    ...
    protected void requestData(int reqCode){};//请求方法处理

    protected void requestTask(int reqCode){
        if (loadingDilog != null){
            showLoadingDilog(null);
    ...
    public void setLoadingDilog(WaitLayer.DialogType type){
        if(loadingDilog != null && loadingDilog.isShow()) loadingDilog.dismiss();
        loadingDilog = new WaitLayer(mActivity, type);
    }

    public void showLoadingDilog(String msg){
        if(loadingDilog != null && !loadingDilog.isShow()){
            loadingDilog.show(msg);
            handler.sendEmptyMessageDelayed(CANCEL, 50000);
        }
    }
    ...
    private void onInitTitleView(View view) {
        titleView = (View)view.findViewById(R.id.header_layout);
    ...
    protected void setHeaderRightText(int id,int colorId, View.OnClickListener listener){
        headerRightText.setVisibility(View.VISIBLE);
        headerRightText.setText(id);
        headerRightText.setTextColor(getResources().getColor(colorId));
        if(listener != null){
            headerRightText.setOnClickListener(listener);
        }
    }
...
//    }
    ...
    protected Response.Listener<Object> createReqSuccessListener(final int reqCode) {
        return new Response.Listener<Object>() {
            ...
            public void onResponse(Object object) {
                ...
                if (object != null){
                    ...
                    if(jsonParserBase != null && Constant.SUCCESS_CODE.equals(jsonParserBase.getCode())){

                        if(requsetListener != null){

                            try {
                                if(!isDetached() && getView() != null )
                                requsetListener.handleRspSuccess(reqCode,object.toString());
                        ...
                        failRespone();
                        toasetUtil.showInfo( jsonParserBase.getData().toString());
                    }
