package com.hyl.hylbase.adapter;
...
import com.hyl.hylbase.utils.ImageLoaderUtil;
 ...
 * 
 */
// pageCount设置缓存的页面数     -预缓存的使用条件 数量>=5
...
public class BannerViewAdapter extends PagerAdapter {
	 ...
	 * @param context
	 */
	public BannerViewAdapter(int id[], View view, Context context) {
	...
	public BannerViewAdapter(ArrayList<BannerImgInfo> bannerImgInfos, Context context,OnClickListener onClickListener,boolean showName) {
		this.onClickListener = onClickListener;
		this.context = context;
		this.bannerImgInfos = bannerImgInfos;
		this.isName = showName;
		m_obj_view =  LayoutInflater.from(context).inflate(R.layout.guide_base, null);
		initBaseView();
	}
...
//		}
		...
		for (int i = 0; i < LENGTH; i++) {
			if(isId){
				...
				views.add(buildImageView(bannerImgInfos.get(i)));
	...
	private View buildImageView(int id) {
		View view = LayoutInflater.from(context).inflate(R.layout.guid_base_page,null);
...
//		buildImgLayout(iv);
	...
	private View buildImageView(BannerImgInfo bannerImgInfo){
		View view = null;
		if(isName){
			view = LayoutInflater.from(context).inflate(R.layout.guid_base_page,null);
		}else {
			view = LayoutInflater.from(context).inflate(R.layout.guid_base_view,null);
		}
		ImageView iv = (ImageView)view.findViewById(R.id.img);
		TextView tv = (TextView)view.findViewById(R.id.name);
		view.setTag(bannerImgInfo.getId());
		if(onClickListener != null){
			view.setOnClickListener(onClickListener);
		}
		if(bannerImgInfo.getThumb() != null){
//			todo 加载网络图片，用ImgeLoader或其他框架
			ImageLoader.getInstance().displayImage(bannerImgInfo.getThumb(), iv, ImageLoaderUtil.mHallIconLoaderOptions);
			tv.setVisibility(View.GONE);
			if(isName){
                tv.setVisibility(View.VISIBLE);
				tv.setText(bannerImgInfo.getTitle());
			}
		}
//		buildImgLayout(iv);
		return view;
